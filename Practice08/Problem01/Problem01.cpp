...
bool IsRepeating(int arr[], int n)
{
	for (int i = 0; i < n * n; i++)
	{
...
int main()
{
	...
	for (int i = 0; i < n * n; i++)
	{
		if (!IsRepeating(mat[i], n) || !IsContaining(mat[i], helperArr, n))
		{
			isSudoku = false;
		}
	}
	...
	for (int i = 0; i < n * n; i++)
	{
		...
		if (!IsRepeating(colArr, n) || !IsContaining(colArr, helperArr, n))
		{
			isSudoku = false;
		}
	}
	...
	for (int i = 0; i < n * n; i += n)
	{
		for (int j = 0; j < n * n; j += n)
		{
			...
			if (!IsRepeating(squareArr, n) || !IsContaining(squareArr, helperArr, n))
			{
				isSudoku = false;
			}
		}
